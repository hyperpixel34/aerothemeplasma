...
MainWindow::MainWindow(QSpinBox *spinbox, QCheckBox *checkbox,
                       QPlainTextEdit *textedit, QPlainTextEdit *customtextedit,
                       QWidget *parent)
  ...
  // Wayland fix.
  // (https://gitgud.io/wackyideas/aerothemeplasma/-/merge_requests/1)
  KWindowEffects::enableBlurBehind(
      this->winId(), true, QRegion(0, 0, this->width(), this->height()));
...
void MainWindow::applyTemporarily() {
  ...
  conf->writeToMemory(exportColor(), ui->kcfg_EnableTransparency->isChecked(),
                      true);
}
...
void MainWindow::changeColor(int index, bool apply) {
  ...
  ui->saturation_Slider->setValue(
      predefined_colors[index].getColor().hsvSaturation());
...
void MainWindow::changeCustomColor(bool apply) {
  if (!preventChanges) {
    ...
    QColor c;
    c.setHsv(ui->hue_Slider->value(), ui->saturation_Slider->value(),
             ui->Lightness_Slider->value(), ui->alpha_slider->value());
...
void MainWindow::applyChanges() {
  cancelChanges = false;
  kcfg_AccentColor->setPlainText(
      exportColor().name(QColor::NameFormat::HexArgb));
  kcfg_AccentColorName->setValue(selected_color);
  kcfg_CustomColor->setPlainText(
      predefined_colors[0].getColor().name(QColor::NameFormat::HexArgb));
...
void MainWindow::closeEvent(QCloseEvent *event) {
  KWin::BlurEffectConfig *conf = (KWin::BlurEffectConfig *)parent();
  conf->writeToMemory(kcfg_AccentColor->toPlainText(),
