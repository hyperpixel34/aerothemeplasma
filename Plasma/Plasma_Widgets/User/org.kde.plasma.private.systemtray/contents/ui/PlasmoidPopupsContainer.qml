        ...
        if (activeApplet != null) {
            ...
            if (activeApplet.fullRepresentationItem instanceof PlasmaComponents3.Page) {
                if (activeApplet.fullRepresentationItem.header && activeApplet.fullRepresentationItem.header instanceof PlasmaExtras.PlasmoidHeading) {
                    ...
                    var header = activeApplet.fullRepresentationItem.header;
                    //console.log("HEADER:" + header.children[1]);
                    //if(header.children[1] !== undefined) {
                    //    var headerHeading = header.children[1];
                    //    if(headerHeading.children[0] !== undefined) {
                    //        var headerLayout = headerHeading.children[0];
                    //        Object.keys(headerLayout).forEach((prop)=> console.log(prop));
                    //        for(var i = 0; i < headerLayout.children.length; i++) {
                    //    console.log("\nCHILD " + i + ":" + headerLayout.children[i]);
                    //    Object.keys(headerLayout.children[i]).forEach((prop)=> console.log(prop));

                    //    /*if(header.children[i].colorGroup !== undefined) {
                    //        header.children[i].colorGroup = 5;
                    //        header.children[i].update();
                    //       console.log(header.children[i].colorGroup);
                    //    }*///
                    //    //if(header.children[i].text !== undefined) console.log(header.children[i].text);
                    //    }
                    //    }
